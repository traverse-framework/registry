//! support.ticket-workspace — put/get/delete a durable ticket workspace via host state.
//!
//! Relative state key is fixed schema property `workspace`. Resource id lives inside the value
//! (`ticket_id`, `status`, `assignee_id`, `tags`). Calls traverse_host::{state_put,state_get,state_delete}.
...
const STATE_KEY: &[u8] = b"workspace";
...
unsafe fn do_put(input: &[u8], out: &mut [u8]) -> usize {
    let Some(workspace) = extract_object_at_depth(input, b"\"workspace\"", 1) else {
        return write_result(
            out,
            false,
            b"invalid_input",
            None,
            None,
            br#"["precondition failed: workspace object required for put"]"#,
        );
    };
    let ticket_id = extract_string_at_depth(workspace, b"\"ticket_id\"", 1);
    let status = extract_string_at_depth(workspace, b"\"status\"", 1);
    let assignee_id = extract_string_at_depth(workspace, b"\"assignee_id\"", 1);
    let tags = extract_array_at_depth(workspace, b"\"tags\"", 1);
    if ticket_id.is_empty() || status.is_empty() || assignee_id.is_empty() || tags.is_none() {
        return write_result(
            out,
            false,
            b"invalid_input",
            None,
            None,
            br#"["precondition failed: workspace.ticket_id/status/assignee_id/tags required"]"#,
        );
    }
...
mod tests {
    ...
    fn happy_put() {
        let out = run(
            r#"{"action":"put","workspace":{"ticket_id":"t-1","status":"open","assignee_id":"agent-1","tags":["billing","p1"]}}"#,
        );
        assert!(out.contains("\"reason_code\":\"ok\""), "{out}");
        assert!(out.contains("\"accepted\":true"));
        assert!(out.contains("\"workspace\":{"));
        assert!(out.contains("\"ticket_id\":\"t-1\""));
    }

    #[test]
    fn happy_put_empty_tags() {
        let out = run(
            r#"{"action":"put","workspace":{"ticket_id":"t-2","status":"open","assignee_id":"agent-1","tags":[]}}"#,
        );
        assert!(out.contains("\"reason_code\":\"ok\""), "{out}");
    }
    ...
    fn happy_get_found() {
        set_get_response(
            r#"{"found":true,"value":{"ticket_id":"t-1","status":"open","assignee_id":"agent-1","tags":["billing"]}}"#,
        );
    ...
    fn rejects_missing_action() {
        let out = run(r#"{"workspace":{"ticket_id":"t-1"}}"#);
    ...
    #[test]
    fn rejects_put_without_workspace() {
    ...
    fn rejects_put_missing_fields() {
        let out = run(
            r#"{"action":"put","workspace":{"ticket_id":"t-1","status":"","assignee_id":"a","tags":[]}}"#,
        );
        assert!(out.contains("\"reason_code\":\"invalid_input\""), "{out}");
    }

    #[test]
    fn rejects_put_missing_tags() {
        let out = run(
            r#"{"action":"put","workspace":{"ticket_id":"t-1","status":"open","assignee_id":"a"}}"#,
        );
        assert!(out.contains("\"reason_code\":\"invalid_input\""), "{out}");
    }
    ...
    fn put_host_error() {
        ...
        let n = unsafe {
            evaluate(
                br#"{"action":"put","workspace":{"ticket_id":"t-1","status":"open","assignee_id":"a","tags":[]}}"#,
                &mut out,
            )
        };
    ...
    fn helpers_edge_cases() {
        ...
        assert!(extract_object_at_depth(br#"{"workspace":null}"#, b"\"workspace\"", 1).is_none());
        assert!(extract_object_at_depth(br#"{"workspace":"x"}"#, b"\"workspace\"", 1).is_none());
        assert_eq!(
            extract_object_at_depth(br#"{"workspace":{"ticket_id":"x"}}"#, b"\"workspace\"", 1),
            Some(br#"{"ticket_id":"x"}"#.as_slice())
        );
        assert_eq!(
            extract_object_at_depth(
                br#"{"workspace":{"note":"a\"b","ticket_id":"x"}}"#,
                b"\"workspace\"",
                1
            ),
            Some(br#"{"note":"a\"b","ticket_id":"x"}"#.as_slice())
        );
        assert_eq!(
            extract_object_at_depth(br#"{"workspace": {"ticket_id":"z"}}"#, b"\"workspace\"", 1),
            Some(br#"{"ticket_id":"z"}"#.as_slice())
        );
        assert_eq!(
            extract_object_at_depth(br#"{"workspace":{"a":{"b":1}}}"#, b"\"workspace\"", 1),
            Some(br#"{"a":{"b":1}}"#.as_slice())
        );
        assert!(extract_object_at_depth(br#"{"workspace":{"ticket_id":"x""#, b"\"workspace\"", 1)
            .is_none());
        assert_eq!(
            extract_array_at_depth(br#"{"tags":["a","b"]}"#, b"\"tags\"", 1),
            Some(br#"["a","b"]"#.as_slice())
        );
        assert_eq!(
            extract_array_at_depth(br#"{"tags":[]}"#, b"\"tags\"", 1),
            Some(br#"[]"#.as_slice())
        );
        assert_eq!(
            extract_array_at_depth(br#"{"tags": ["a"]}"#, b"\"tags\"", 1),
            Some(br#"["a"]"#.as_slice())
        );
        assert_eq!(
            extract_array_at_depth(br#"{"tags":["a\"b","c"]}"#, b"\"tags\"", 1),
            Some(br#"["a\"b","c"]"#.as_slice())
        );
        assert!(extract_array_at_depth(br#"{"tags":"x"}"#, b"\"tags\"", 1).is_none());
        assert!(extract_array_at_depth(br#"{"tags":["a""#, b"\"tags\"", 1).is_none());
