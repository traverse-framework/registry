//! commerce.pricing-config — put/get/delete durable state via host state_*.
//!
//! Relative state key is fixed schema property `config`. Resource ids live inside the value.
...
unsafe fn do_put(input: &[u8], out: &mut [u8]) -> usize {
    let Some(session) = extract_object_at_depth(input, b"\"config\"", 1) else {
        return write_result(
            out,
            false,
            b"invalid_input",
    ...
    let config_id = extract_string_at_depth(session, b"\"config_id\"", 1);
    let currency = extract_string_at_depth(session, b"\"currency\"", 1);
    let rates_json = extract_string_at_depth(session, b"\"rates_json\"", 1);
        ...
        || rates_json.is_empty()
    {
        return write_result(
            ...
            None,
            None,
            br#"["precondition failed: config.config_id/currency/rates_json required"]"#,
        );
    }
...
mod tests {
    ...
    fn happy_put() {
        let out = run(
            r#"{"action":"put","config":{"config_id":"price-1","currency":"USD","rates_json":"base=1.0"}}"#,
        );
        assert!(out.contains("\"reason_code\":\"ok\""), "{out}");
        assert!(out.contains("\"accepted\":true"));
        assert!(out.contains("\"config\":{"));
        assert!(out.contains("\"config_id\":\"price-1\""));
    }

    #[test]
    fn happy_get_found() {
        set_get_response(
            r#"{"found":true,"value":{"config_id":"price-1","currency":"USD","rates_json":"base=1.0"}}"#,
        );
    ...
    fn rejects_missing_action() {
        let out = run(r#"{"config":{"config_id":"price-1"}}"#);
    ...
    fn rejects_put_missing_fields() {
        let out = run(
            r#"{"action":"put","config":{"config_id":"price-1","currency":"","rates_json":"base=1.0"}}"#,
        );
        assert!(out.contains("\"reason_code\":\"invalid_input\""), "{out}");
    }
    ...
    fn put_host_error() {
        ...
        let n = unsafe {
            evaluate(
                br#"{"action":"put","config":{"config_id":"price-1","currency":"USD","rates_json":"base=1.0"}}"#,
                &mut out,
            )
        };
    ...
    fn helpers_edge_cases() {
	 ...
	 \"ok\""), b"ok");
        ...
        assert!(
            extract_object_at_depth(br#"{"config":{"note":"a\"b","config_id":"x"}}"#, b"\"config\"", 1)
                .is_some()
        );
        assert!(extract_object_at_depth(br#"{"config":{"config_id":"x""#, b"\"config\"", 1).is_none());
