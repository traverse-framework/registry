//! commerce.pricing-config — put/get/delete a durable pricing config via host state.
//!
//! Relative state key is fixed schema property `config`. Resource id lives inside the value
//! (`config_id`, `currency`, `rates`). Calls traverse_host::{state_put,state_get,state_delete}.
...
unsafe fn do_put(input: &[u8], out: &mut [u8]) -> usize {
    let Some(config) = extract_object_at_depth(input, b"\"config\"", 1) else {
        return write_result(
            out,
            false,
            b"invalid_input",
            None,
            None,
            br#"["precondition failed: config object required for put"]"#,
        );
    };
    let config_id = extract_string_at_depth(config, b"\"config_id\"", 1);
    let currency = extract_string_at_depth(config, b"\"currency\"", 1);
    let rates = extract_object_at_depth(config, b"\"rates\"", 1);
    if config_id.is_empty() || currency.is_empty() || rates.is_none() {
        return write_result(
            out,
            false,
            b"invalid_input",
            None,
            None,
            br#"["precondition failed: config.config_id/currency/rates required"]"#,
        );
    }
...
fn write_result(
    ...
    if let Some(config_val) = config {
        i = copy(out, i, b",\"config\":");
        i = copy(out, i, config_val);
    }
...
mod tests {
    ...
    fn happy_put() {
        let out = run(
            r#"{"action":"put","config":{"config_id":"pc-1","currency":"USD","rates":{"base":1.0,"vip":0.9}}}"#,
        );
        assert!(out.contains("\"reason_code\":\"ok\""), "{out}");
        assert!(out.contains("\"accepted\":true"));
        assert!(out.contains("\"config\":{"));
        assert!(out.contains("\"config_id\":\"pc-1\""));
    }

    #[test]
    fn happy_put_empty_rates() {
        let out = run(
            r#"{"action":"put","config":{"config_id":"pc-2","currency":"EUR","rates":{}}}"#,
        );
        assert!(out.contains("\"reason_code\":\"ok\""), "{out}");
    }
    ...
    fn happy_get_found() {
        set_get_response(
            r#"{"found":true,"value":{"config_id":"pc-1","currency":"USD","rates":{"base":1.0}}}"#,
        );
    ...
    fn rejects_missing_action() {
        let out = run(r#"{"config":{"config_id":"pc-1"}}"#);
    ...
    fn rejects_put_missing_fields() {
        let out = run(
            r#"{"action":"put","config":{"config_id":"pc-1","currency":"","rates":{}}}"#,
        );
        assert!(out.contains("\"reason_code\":\"invalid_input\""), "{out}");
    }

    #[test]
    fn rejects_put_missing_rates() {
        let out = run(
            r#"{"action":"put","config":{"config_id":"pc-1","currency":"USD"}}"#,
        );
        assert!(out.contains("\"reason_code\":\"invalid_input\""), "{out}");
    }
    ...
    fn put_host_error() {
        ...
        let n = unsafe {
            evaluate(
                br#"{"action":"put","config":{"config_id":"pc-1","currency":"USD","rates":{"base":1.0}}}"#,
                &mut out,
            )
        };
    ...
    fn helpers_edge_cases() {
        ...
        assert!(extract_object_at_depth(br#"{"config":null}"#, b"\"config\"", 1).is_none());
        assert!(extract_object_at_depth(br#"{"config":"x"}"#, b"\"config\"", 1).is_none());
        assert_eq!(
            extract_object_at_depth(br#"{"config":{"config_id":"x"}}"#, b"\"config\"", 1),
            Some(br#"{"config_id":"x"}"#.as_slice())
        );
        assert_eq!(
            extract_object_at_depth(
                br#"{"config":{"note":"a\"b","config_id":"x"}}"#,
                b"\"config\"",
                1
            ),
            Some(br#"{"note":"a\"b","config_id":"x"}"#.as_slice())
        );
        assert_eq!(
            extract_object_at_depth(br#"{"config": {"config_id":"z"}}"#, b"\"config\"", 1),
            Some(br#"{"config_id":"z"}"#.as_slice())
        );
        assert_eq!(
            extract_object_at_depth(br#"{"config":{"a":{"b":1}}}"#, b"\"config\"", 1),
            Some(br#"{"a":{"b":1}}"#.as_slice())
        );
        assert!(extract_object_at_depth(br#"{"config":{"config_id":"x""#, b"\"config\"", 1)
            .is_none());
        assert_eq!(
            extract_object_at_depth(br#"{"rates":{"base":1.0}}"#, b"\"rates\"", 1),
            Some(br#"{"base":1.0}"#.as_slice())
        );
        assert_eq!(
            extract_object_at_depth(br#"{"rates":{}}"#, b"\"rates\"", 1),
            Some(br#"{}"#.as_slice())
        );
        assert_eq!(
            extract_object_at_depth(br#"{"rates": {"vip":0.9}}"#, b"\"rates\"", 1),
            Some(br#"{"vip":0.9}"#.as_slice())
        );
        assert!(extract_object_at_depth(br#"{"rates":"x"}"#, b"\"rates\"", 1).is_none());
        assert!(extract_object_at_depth(br#"{"rates":{"base":1.0"#, b"\"rates\"", 1).is_none());
        assert_eq!(
        ...
        let with_obj = br#"{"rates":{"base":1.0},"action":"get"}"#;
        assert_eq!(extract_string_at_depth(with_obj, b"\"action\"", 1), b"get");
