//! identity.principal-session — put/get/delete durable state via host state_*.
//!
//! Relative state key is fixed schema property `session`. Resource ids live inside the value.
...
unsafe fn do_put(input: &[u8], out: &mut [u8]) -> usize {
        ...
        || issued_at.is_empty()
    {
        return write_result(
            ...
            None,
            None,
            br#"["precondition failed: session.session_id/principal_id/scopes_json/issued_at required"]"#,
        );
    }
...
mod tests {
    ...
    fn happy_put() {
        let out = run(
            r#"{"action":"put","session":{"session_id":"sess-1","principal_id":"user-ada","scopes_json":"read,write","issued_at":"2026-09-08T20:00:00Z"}}"#,
        );
    ...
    fn happy_get_found() {
        set_get_response(
            r#"{"found":true,"value":{"session_id":"sess-1","principal_id":"user-ada","scopes_json":"read,write","issued_at":"2026-09-08T20:00:00Z"}}"#,
        );
    ...
    fn rejects_missing_action() {
        let out = run(r#"{"session":{"session_id":"sess-1"}}"#);
    ...
    fn rejects_put_missing_fields() {
        let out = run(
            r#"{"action":"put","session":{"session_id":"sess-1","principal_id":"","scopes_json":"read,write","issued_at":"2026-09-08T20:00:00Z"}}"#,
        );
        assert!(out.contains("\"reason_code\":\"invalid_input\""), "{out}");
    }
    ...
    fn put_host_error() {
        ...
        let n = unsafe {
            evaluate(
                br#"{"action":"put","session":{"session_id":"sess-1","principal_id":"user-ada","scopes_json":"read,write","issued_at":"2026-09-08T20:00:00Z"}}"#,
                &mut out,
            )
        };
    ...
    fn helpers_edge_cases() {
	 ...
	 \"ok\""), b"ok");
        ...
        assert!(
            extract_object_at_depth(br#"{"session":{"note":"a\"b","session_id":"x"}}"#, b"\"session\"", 1)
                .is_some()
        );
        assert!(extract_object_at_depth(br#"{"session":{"session_id":"x""#, b"\"session\"", 1).is_none());
