//! identity.challenge-session — put/get/delete durable state via host state_*.
//!
//! Relative state key is fixed schema property `challenge`. Resource ids live inside the value.
...
unsafe fn do_put(input: &[u8], out: &mut [u8]) -> usize {
    let Some(session) = extract_object_at_depth(input, b"\"challenge\"", 1) else {
        return write_result(
            out,
            false,
            b"invalid_input",
    ...
    let challenge_id = extract_string_at_depth(session, b"\"challenge_id\"", 1);
    let principal_hint = extract_string_at_depth(session, b"\"principal_hint\"", 1);
    let method = extract_string_at_depth(session, b"\"method\"", 1);
    let expires_at = extract_string_at_depth(session, b"\"expires_at\"", 1);
    let status = extract_string_at_depth(session, b"\"status\"", 1);
...
mod tests {
    ...
    fn happy_put() {
        let out = run(
            r#"{"action":"put","challenge":{"challenge_id":"chal-1","principal_hint":"ada@example.com","method":"otp","expires_at":"2026-09-09T00:00:00Z","status":"pending"}}"#,
        );
        assert!(out.contains("\"reason_code\":\"ok\""), "{out}");
        assert!(out.contains("\"accepted\":true"));
        assert!(out.contains("\"challenge\":{"));
        assert!(out.contains("\"challenge_id\":\"chal-1\""));
    }

    #[test]
    fn happy_get_found() {
        set_get_response(
            r#"{"found":true,"value":{"challenge_id":"chal-1","principal_hint":"ada@example.com","method":"otp","expires_at":"2026-09-09T00:00:00Z","status":"pending"}}"#,
        );
    ...
    fn rejects_missing_action() {
        let out = run(r#"{"challenge":{"challenge_id":"chal-1"}}"#);
    ...
    fn rejects_put_missing_fields() {
        let out = run(
            r#"{"action":"put","challenge":{"challenge_id":"chal-1","principal_hint":"","method":"otp","expires_at":"2026-09-09T00:00:00Z","status":"pending"}}"#,
        );
        assert!(out.contains("\"reason_code\":\"invalid_input\""), "{out}");
    }
    ...
    fn put_host_error() {
        ...
        let n = unsafe {
            evaluate(
                br#"{"action":"put","challenge":{"challenge_id":"chal-1","principal_hint":"ada@example.com","method":"otp","expires_at":"2026-09-09T00:00:00Z","status":"pending"}}"#,
                &mut out,
            )
        };
    ...
    fn helpers_edge_cases() {
	 ...
	 \"ok\""), b"ok");
        ...
        assert!(
            extract_object_at_depth(br#"{"challenge":{"note":"a\"b","challenge_id":"x"}}"#, b"\"challenge\"", 1)
                .is_some()
        );
        assert!(extract_object_at_depth(br#"{"challenge":{"challenge_id":"x""#, b"\"challenge\"", 1).is_none());
