//! identity.challenge-session — put/get/delete a durable challenge via host state.
//!
//! Relative state key is fixed schema property `challenge`. Resource id lives inside the value
//! (`challenge_id`, `principal_hint`, `method`, `expires_at`, `status`). Calls traverse_host::{state_put,state_get,state_delete}.
...
const STATE_KEY: &[u8] = b"challenge";
...
unsafe fn do_put(input: &[u8], out: &mut [u8]) -> usize {
    let Some(challenge) = extract_object_at_depth(input, b"\"challenge\"", 1) else {
        return write_result(
            out,
            false,
            b"invalid_input",
            None,
            None,
            br#"["precondition failed: challenge object required for put"]"#,
        );
    };
    let challenge_id = extract_string_at_depth(challenge, b"\"challenge_id\"", 1);
    let principal_hint = extract_string_at_depth(challenge, b"\"principal_hint\"", 1);
    let method = extract_string_at_depth(challenge, b"\"method\"", 1);
    let expires_at = extract_string_at_depth(challenge, b"\"expires_at\"", 1);
    let status = extract_string_at_depth(challenge, b"\"status\"", 1);
    if challenge_id.is_empty()
        || principal_hint.is_empty()
        || method.is_empty()
        || expires_at.is_empty()
        || status.is_empty()
    {
        return write_result(
            ...
            None,
            None,
            br#"["precondition failed: challenge.challenge_id/principal_hint/method/expires_at/status required"]"#,
        );
    }
...
fn write_result(
    ...
    if let Some(val) = challenge {
        i = copy(out, i, b",\"challenge\":");
        i = copy(out, i, val);
    }
...
mod tests {
    ...
    #[test]
    fn happy_put() {
        let out = run(r#"{"action":"put","challenge":{"challenge_id":"ch-1","principal_hint":"user@example.com","method":"email_otp","expires_at":"2026-09-08T21:00:00Z","status":"pending"}}"#);
        assert!(out.contains("\"reason_code\":\"ok\""), "{out}");
        assert!(out.contains("\"accepted\":true"));
        assert!(out.contains("\"challenge\":{"));
        assert!(out.contains("\"challenge_id\":\"ch-1\""));
    }

    #[test]
    fn happy_get_found() {
        set_get_response(r#"{"found":true,"value":{"challenge_id":"ch-1","principal_hint":"user@example.com","method":"email_otp","expires_at":"2026-09-08T21:00:00Z","status":"pending"}}"#);
    ...
    fn rejects_missing_action() {
        let out = run(r#"{"challenge":{"challenge_id":"x"}}"#);
    ...
    #[test]
    fn rejects_put_without_challenge() {
    ...
    fn rejects_put_missing_fields() {
        let out = run(r#"{"action":"put","challenge":{"challenge_id":"ch-1","principal_hint":"","method":"email_otp","expires_at":"2026-09-08T21:00:00Z","status":"pending"}}"#);
        assert!(out.contains("\"reason_code\":\"invalid_input\""), "{out}");
    }
    ...
    fn put_host_error() {
        ...
        let n = unsafe {
            evaluate(
                br#"{"action":"put","challenge":{"challenge_id":"ch-1","principal_hint":"user@example.com","method":"email_otp","expires_at":"2026-09-08T21:00:00Z","status":"pending"}}"#,
                &mut out,
            )
        };
    ...
    fn helpers_edge_cases() {
        ...
        assert!(extract_object_at_depth(br#"{"challenge":null}"#, b"\"challenge\"", 1).is_none());
        assert!(extract_object_at_depth(br#"{"challenge":"x"}"#, b"\"challenge\"", 1).is_none());
        assert_eq!(
            extract_object_at_depth(br#"{"challenge":{"challenge_id":"x"}}"#, b"\"challenge\"", 1),
            Some(br#"{"challenge_id":"x"}"#.as_slice())
        );
        assert_eq!(
            extract_object_at_depth(
                br#"{"challenge":{"note":"a\"b","challenge_id":"x"}}"#,
                b"\"challenge\"",
                1
            ),
            Some(br#"{"note":"a\"b","challenge_id":"x"}"#.as_slice())
        );
        assert_eq!(
            extract_object_at_depth(br#"{"challenge": {"challenge_id":"z"}}"#, b"\"challenge\"", 1),
            Some(br#"{"challenge_id":"z"}"#.as_slice())
        );
        assert_eq!(
            extract_object_at_depth(br#"{"challenge":{"a":{"b":1}}}"#, b"\"challenge\"", 1),
            Some(br#"{"a":{"b":1}}"#.as_slice())
        );
        assert!(extract_object_at_depth(br#"{"challenge":{"challenge_id":"x""#, b"\"challenge\"", 1)
            .is_none());
        assert_eq!(
