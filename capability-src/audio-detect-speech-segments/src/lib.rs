//! Silero VAD (16k) single-chunk speech-risk scoring engine, shared between
//! `audio.detect-speech-segments`'s own `main.rs` binary (thin JSON-wiring
//! shim over this crate) and `audio.transcribe-speech` (registry#477),
//! which reuses [`Model`]/[`score_chunk`] directly as a pre-check before
//! running its own, much more expensive Whisper encoder/decoder -- Whisper
//! has no notion of "is this speech at all" and will confidently transcribe
//! silence or a pure tone as plausible-sounding text (observed directly:
//! `"[MUSIC PLAYING]"` for a synthetic sine wave) unless something upstream
//! tells it not to bother.
//!
//! One [`score_chunk`] call scores exactly one 512-sample (32ms) window of
//! 16kHz mono PCM, threading the model's own streaming state (a 64-sample
//! audio "context" carried from the previous window, plus an LSTM `(h, c)`
//! pair) explicitly through the call -- callers chunk a real recording and
//! accumulate probabilities into speech-risk segments (their own policy,
//! e.g. threshold + hysteresis, matching how `classifier.
...
// ---------------------------------------------------------------------
// Capability request/response wiring (audio.detect-speech-segments' own
// JSON shape). Not used by audio.transcribe-speech's internal VAD
// pre-check, which calls `score_chunk` directly on raw f32 samples.
// ---------------------------------------------------------------------
...
pub fn load_production_model() -> Model {
...
/// A small, deterministic-but-varied, structurally-valid `VAD1` model --
/// same fixture this crate's own tests use, exposed so a dependent crate
/// (e.g. `audio.transcribe-speech`, registry#477, which reuses [`score_chunk`]
/// as a speech-presence pre-check) can exercise real plumbing through this
/// engine in its own tests without embedding the real ~1.2 MB weight table.
/// NOT a claim of real VAD accuracy -- that's verified separately against
/// the published `full-model` artifact and the real ONNX graph.
...
// dimensions (this model's architecture isn't configurable, so a "tiny"
// fixture isn't smaller than the real one) -- exercises the
// parser/conv/LSTM/sigmoid plumbing. NOT a claim of real VAD accuracy;
// that's verified separately against the published `full-model` artifact
// and the real ONNX graph (see the publish PR). Always compiled (not
// `cfg(test)`-gated) so `fixture_model()` above can hand it to a dependent
// crate's own tests, not just this crate's.
// ---------------------------------------------------------------------
