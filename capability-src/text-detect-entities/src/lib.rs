...
pub fn encode(model: &Model, text: &str) -> Vec<Token> {
...
pub fn forward_logits(model: &Model, tokens: &[Token]) -> Vec<Vec<f32>> {
    let seq = tokens.len();
...
pub fn load_production_model() -> Model<'static> {
...
// 11-token cased vocab. Exercises parsing/tokenizer/forward/BIO-decode
...
// ONNX graph (see the publish PR). Always compiled (not `cfg(test)`-gated)
// so `fixture_model()` below can hand it to a dependent crate's own tests
// too (e.g. `text.redact-entities`, registry#481, which reuses this
// crate's encoder/decoder engine directly rather than duplicating it).
// ---------------------------------------------------------------------
...
fn fixture_bin() -> Vec<u8> {
    let tokens = [
        "[PAD]", "[UNK]", "[CLS]", "[SEP]", "hello", "world", "Musk", "El", "##on", "##sk", "Mu",
    ];
...
/// A small, deterministic-but-varied, structurally-valid `NER1` model --
/// same fixture this crate's own tests use, exposed (via a leaked, `'static`
/// buffer -- fine for a one-shot WASI command / short-lived test process)
/// so a dependent crate can exercise real plumbing through this engine in
/// its own tests without embedding the real ~65.6 MB weight table.
pub fn fixture_model() -> Model<'static> {
    let bin = alloc::boxed::Box::leak(fixture_bin().into_boxed_slice());
