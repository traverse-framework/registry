...
pub extern "C" fn _start() { unsafe {
    ...
    let len = decide(&INPUT[..total], &mut OUTPUT);
    let mut written = 0usize;
    let v = IoVec { buffer: OUTPUT.as_ptr(), length: len };
    let _ = fd_write(1, &v, 1, &mut written);
} }

fn decide(request: &[u8], out: &mut [u8]) -> usize {
    let current_state = string_after(request, b"\"current_state\"");
    let review_decision = string_after(request, b"\"review_decision\"");
    ...
    if current_state.is_empty() { return error_json(out, b"current_state is required"); }
    if review_decision.is_empty() { return error_json(out, b"review_decision is required"); }
    if policy_version.is_empty() { return error_json(out, b"policy.version is required"); }

    if review_decision == b"verify" && current_state != b"provisional" {
        return invalid_transition_json(
            out,
            policy_version,
            b"verify_requires_provisional",
            b"only provisional items may be verified",
        );
    }

    let (next_state, reason_code): (&[u8], &[u8]) =
        if review_decision == b"verify" {
            (b"verified", b"verified_from_provisional")
        } else if review_decision == b"reject" {
            (b"rejected", b"rejected_by_reviewer")
        } else {
            (b"provisional", b"provisional_by_reviewer")
        };

    let mut at = 0usize;
    at = copy(out, at, b"{\"status\":\"applied\",\"next_state\":\"");
    at = copy(out, at, next_state);
...
fn error_json(out: &mut [u8], message: &[u8]) -> usize {
    let mut at = 0usize;
    at = copy(out, at, b"{\"status\":\"invalid_transition\",\"policy_version\":\"input-validation\",\"error_code\":\"verify_requires_provisional\",\"error_message\":\"");
    at = copy_json(out, at, message);
    copy(out, at, b"\"}")
}
fn invalid_transition_json(out: &mut [u8], policy_version: &[u8], code: &[u8], message: &[u8]) -> usize {
    let mut at = 0usize;
    at = copy(out, at, b"{\"status\":\"invalid_transition\",\"policy_version\":\"");
    at = copy_json(out, at, policy_version);
    at = copy(out, at, b"\",\"error_code\":\"");
    at = copy_json(out, at, code);
    at = copy(out, at, b"\",\"error_message\":\"");
    at = copy_json(out, at, message);
    copy(out, at, b"\"}")
}
fn skip(mut s: &[u8]) -> &[u8] {
...
mod tests {
    ...
    fn exercises_runtime_fixture_1() {
        let input = include_bytes!("../tests/uc01-verify.json");
        let mut out = [0u8; 65_536];
        let length = decide(input, &mut out);
        assert!(length > 0);
        assert!(length <= out.len());
    }
    #[test]
    fn exercises_runtime_fixture_2() {
        let input = include_bytes!("../tests/uc02-invalid-verify.json");
        let mut out = [0u8; 65_536];
        let length = decide(input, &mut out);
        assert!(length > 0);
        assert!(length <= out.len());
    ...
    fn exercises_runtime_fixture_3() {
        let input = include_bytes!("../tests/uc03-reject.json");
        let mut out = [0u8; 65_536];
        let length = decide(input, &mut out);
        assert!(length > 0);
        assert!(length <= out.len());
    ...
    fn exercises_runtime_fixture_4() {
        let input = include_bytes!("../tests/uc04-provisional.json");
        let mut out = [0u8; 65_536];
        let length = decide(input, &mut out);
        assert!(length > 0);
        assert!(length <= out.len());
    ...
    fn remains_total_for_truncated_and_corrupted_real_requests() {
        for fixture in [include_bytes!("../tests/uc01-verify.json").as_slice(), include_bytes!("../tests/uc02-invalid-verify.json").as_slice(), include_bytes!("../tests/uc03-reject.json").as_slice(), include_bytes!("../tests/uc04-provisional.json").as_slice()] {
            for end in 0..=fixture.len() {
                let mut out = [0u8; 65_536];
                let length = decide(&fixture[..end], &mut out);
                assert!(length <= out.len());
            }
            for index in 0..core::cmp::min(fixture.len(), 512) {
                ...
                corrupted[index] = b'?';
                let mut out = [0u8; 65_536];
                let length = decide(&corrupted, &mut out);
                assert!(length <= out.len());
            }
        }
    }
