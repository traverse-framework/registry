//! core.followup-session — put/get/delete a follow-up working session via host state.
//!
//! Relative state key is fixed schema property `session`. Resource id lives inside the value
//! (`session_id`, `agenda_item_ids`, `cursor`). Calls traverse_host::{state_put,state_get,state_delete}.
...
unsafe fn do_put(input: &[u8], out: &mut [u8]) -> usize {
    ...
    if session_id.is_empty() {
        return write_result(
            ...
            None,
            None,
            br#"["precondition failed: session.session_id required"]"#,
        );
    }
    if extract_i64(session, b"\"cursor\"").is_none() {
        return write_result(
            out,
            false,
            b"invalid_input",
            None,
            None,
            br#"["precondition failed: session.cursor required as integer"]"#,
        );
    }
    if !has_array_key(session, b"\"agenda_item_ids\"") {
        return write_result(
            out,
            false,
            b"invalid_input",
            None,
            None,
            br#"["precondition failed: session.agenda_item_ids array required"]"#,
        );
    }
...
fn has_array_key(hay: &[u8], key: &[u8]) -> bool {
    let Some(pos) = find_key_at_depth(hay, key, 1) else {
        return false;
    };
...
mod tests {
    ...
    fn happy_put() {
        let out = run(
            r#"{"action":"put","session":{"session_id":"sess-1","agenda_item_ids":["a1","a2"],"cursor":0}}"#,
        );
    ...
    fn happy_get_found() {
        set_get_response(
            r#"{"found":true,"value":{"session_id":"sess-1","agenda_item_ids":["a1"],"cursor":1}}"#,
        );
    ...
    fn rejects_missing_action() {
        let out = run(r#"{"session":{"session_id":"s"}}"#);
    ...
    #[test]
    fn rejects_put_empty_session_id() {
        let out = run(
            r#"{"action":"put","session":{"session_id":"","agenda_item_ids":[],"cursor":0}}"#,
        );
        assert!(out.contains("\"reason_code\":\"invalid_input\""), "{out}");
    }
    ...
    #[test]
    fn rejects_put_missing_cursor() {
        let out = run(
            r#"{"action":"put","session":{"session_id":"sess-1","agenda_item_ids":[]}}"#,
        );
        assert!(out.contains("\"reason_code\":\"invalid_input\""), "{out}");
    }

    #[test]
    fn rejects_put_missing_agenda() {
        let out = run(r#"{"action":"put","session":{"session_id":"sess-1","cursor":0}}"#);
        assert!(out.contains("\"reason_code\":\"invalid_input\""), "{out}");
    }
    ...
    fn put_host_error() {
        ...
        let n = unsafe {
            evaluate(
                br#"{"action":"put","session":{"session_id":"sess-1","agenda_item_ids":[],"cursor":0}}"#,
                &mut out,
            )
        };
    ...
    fn helpers_edge_cases() {
        assert!(has_array_key(
            br#"{"agenda_item_ids":["a"]}"#,
            b"\"agenda_item_ids\""
        ));
        assert!(has_array_key(
            b"{\"agenda_item_ids\":\n\t []}",
        ...
        assert!(!has_array_key(
            br#"{"agenda_item_ids":"x"}"#,
            b"\"agenda_item_ids\""
        ));
        assert!(!has_array_key(br#"{"other":[]}"#, b"\"agenda_item_ids\""));
        assert!(!has_array_key(br#"{"agenda_item_ids"}"#, b"\"agenda_item_ids\""));
        assert_eq!(trim_c_str(b"{\"a\":1}\0"), br#"{"a":1}"#);
        assert_eq!(trim_c_str(b"no-brace\0"), b"no-brace");
        assert_eq!(trim_c_str(br#"{"a":"b\"c"}"#), br#"{"a":"b\"c"}"#);
        let mut buf = [0u8; 8];
        let n = copy_usize(&mut buf, 0, 7);
        assert_eq!(&buf[..n], b"7");
        let n0 = copy_usize(&mut buf, 0, 0);
        assert_eq!(&buf[..n0], b"0");
        let mut tiny = [0u8; 1];
        assert_eq!(copy_usize(&mut tiny, 0, 12), 0);
        assert_eq!(copy(&mut tiny, 0, b"ab"), 0);
        assert_eq!(extract_i64(b"\"cursor\": -3", b"\"cursor\""), Some(-3));
        assert_eq!(extract_i64(b"\"cursor\":\n 4", b"\"cursor\""), Some(4));
        assert_eq!(extract_i64(b"\"cursor\":", b"\"cursor\""), None);
        assert_eq!(extract_i64(b"\"cursor\": true", b"\"cursor\""), None);
        ...
        assert!(extract_object_at_depth(br#"{"session":{"id":1"#, b"\"session\"", 1).is_none());
        assert_eq!(
            extract_object_at_depth(br#"{"session": {"session_id":"s"}}"#, b"\"session\"", 1),
            Some(br#"{"session_id":"s"}"#.as_slice())
        );
        assert_eq!(
            extract_object_at_depth(br#"{"session":{"note":"a\"b","session_id":"s"}}"#, b"\"session\"", 1),
            Some(br#"{"note":"a\"b","session_id":"s"}"#.as_slice())
        );
