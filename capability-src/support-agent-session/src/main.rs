//! support.agent-session — put/get/delete a durable agent handling session via host state.
//!
//! Relative state key is fixed schema property `session`. Resource id lives inside the value
//! (`session_id`, `agent_id`, `active_ticket_id`, `started_at`). Calls traverse_host::{state_put,state_get,state_delete}.
...
unsafe fn do_put(input: &[u8], out: &mut [u8]) -> usize {
    ...
    let agent_id = extract_string_at_depth(session, b"\"agent_id\"", 1);
    let active_ticket_id = extract_string_at_depth(session, b"\"active_ticket_id\"", 1);
    let started_at = extract_string_at_depth(session, b"\"started_at\"", 1);
    if session_id.is_empty()
        || agent_id.is_empty()
        || active_ticket_id.is_empty()
        || started_at.is_empty()
    {
        return write_result(
            out,
            ...
            None,
            None,
            br#"["precondition failed: session.session_id/agent_id/active_ticket_id/started_at required"]"#,
        );
    }
...
mod tests {
    ...
    fn happy_put() {
        let out = run(
            r#"{"action":"put","session":{"session_id":"s-1","agent_id":"agent-1","active_ticket_id":"t-1","started_at":"2026-09-08T20:00:00Z"}}"#,
        );
        assert!(out.contains("\"reason_code\":\"ok\""), "{out}");
        assert!(out.contains("\"accepted\":true"));
        assert!(out.contains("\"session\":{"));
        assert!(out.contains("\"session_id\":\"s-1\""));
    }

    #[test]
    fn happy_get_found() {
        set_get_response(
            r#"{"found":true,"value":{"session_id":"s-1","agent_id":"agent-1","active_ticket_id":"t-1","started_at":"2026-09-08T20:00:00Z"}}"#,
        );
    ...
    fn rejects_missing_action() {
        let out = run(r#"{"session":{"session_id":"s-1"}}"#);
    ...
    fn rejects_put_missing_fields() {
        let out = run(
            r#"{"action":"put","session":{"session_id":"s-1","agent_id":"","active_ticket_id":"t-1","started_at":"2026-09-08T20:00:00Z"}}"#,
        );
        assert!(out.contains("\"reason_code\":\"invalid_input\""), "{out}");
    }
    ...
    fn put_host_error() {
        ...
        let n = unsafe {
            evaluate(
                br#"{"action":"put","session":{"session_id":"s-1","agent_id":"a","active_ticket_id":"t","started_at":"2026-09-08T20:00:00Z"}}"#,
                &mut out,
            )
        };
