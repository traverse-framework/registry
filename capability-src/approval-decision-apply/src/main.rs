...
pub extern "C" fn _start() { unsafe {
    ...
    let len = apply(&INPUT[..total], &mut OUTPUT);
    let mut written = 0usize;
    let v = IoVec { buffer: OUTPUT.as_ptr(), length: len };
    let _ = fd_write(1, &v, 1, &mut written);
} }

fn apply(request: &[u8], out: &mut [u8]) -> usize {
    let proposal = match object_after(request, b"\"proposal\"") { Some(v) => v, None => return error_json(out, b"proposal.id is required") };
    let approval = match object_after(request, b"\"approval\"") { Some(v) => v, None => return error_json(out, b"approval.decision is required") };
    ...
    if proposal_id.is_empty() { return error_json(out, b"proposal.id is required"); }
    if reviewer_id.is_empty() { return error_json(out, b"approval.reviewer_id is required"); }
    if decision.is_empty() { return error_json(out, b"approval.decision is required"); }
    ...
    if decision != b"approve" {
        ...
        at = copy(out, at, b"\",\"reason\":\"human_approval_required\",\"policy_version\":\"");
        at = copy_json(out, at, policy_version);
        return copy(out, at, b"\"}");
    }

    let mut at = 0usize;
    at = copy(out, at, b"{\"applied\":true,\"proposal_id\":\"");
    at = copy_json(out, at, proposal_id);
...
mod tests {
    ...
    fn exercises_runtime_fixture_1() {
        ...
        let length = apply(input, &mut out);
        assert!(length > 0);
        assert!(length <= out.len());
    }
    #[test]
    fn exercises_runtime_fixture_2() {
        let input = include_bytes!("../tests/uc02-deny.json");
        let mut out = [0u8; 65_536];
        let length = apply(input, &mut out);
        assert!(length > 0);
        assert!(length <= out.len());
    ...
    fn remains_total_for_truncated_and_corrupted_real_requests() {
        for fixture in [include_bytes!("../tests/uc01-approve.json").as_slice(), include_bytes!("../tests/uc02-deny.json").as_slice()] {
            for end in 0..=fixture.len() {
                let mut out = [0u8; 65_536];
                let length = apply(&fixture[..end], &mut out);
                assert!(length <= out.len());
            }
            for index in 0..core::cmp::min(fixture.len(), 512) {
                ...
                corrupted[index] = b'?';
                let mut out = [0u8; 65_536];
                let length = apply(&corrupted, &mut out);
                assert!(length <= out.len());
            }
        }
    }
