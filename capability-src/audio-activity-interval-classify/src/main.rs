...
mod tests {
    ...
    fn classify_json(input: &[u8]) -> String {
        let mut output = [0u8; OUTPUT_LIMIT];
        let length = classify(input, &mut output);
        String::from_utf8(output[..length].to_vec()).unwrap()
    }

    #[test]
    fn merges_adjacent_windows_and_preserves_transitions() {
        let result = classify_json(br#"{"windows":[{"start_ms":0,"end_ms":10,"quality":"quiet"},{"start_ms":10,"end_ms":20,"quality":"quiet"},{"start_ms":25,"end_ms":40,"quality":"clipped"}]}"#);
        assert!(result.contains("\"start_ms\":0"));
        assert!(result.contains("\"end_ms\":20"));
        assert!(result.contains("\"activity\":\"silence\""));
        assert!(result.contains("\"activity\":\"clipped\""));
        assert!(result.contains("\"represented_window_count\":3"));
    }

    #[test]
    fn classifies_background_and_possible_events() {
        let result = classify_json(br#"{"windows":[{"start_ms":1,"end_ms":2,"quality":"background_dominant"},{"start_ms":2,"end_ms":3,"quality":"active"}]}"#);
        assert!(result.contains("background"));
        assert!(result.contains("possible-event"));
    }

    #[test]
    fn rejects_bad_intervals_and_window_limits() {
        assert!(classify_json(br#"{"windows":[{"start_ms":4,"end_ms":4}]}"#).contains("invalid_window_interval"));
        assert!(classify_json(br#"{"windows":[{"start_ms":-1,"end_ms":2}]}"#).contains("invalid_window_interval"));
        let mut input = String::from("{\"windows\":[");
        for i in 0..=WINDOW_LIMIT { if i > 0 { input.push(','); } input.push_str(&format!("{{\"start_ms\":{},\"end_ms\":{} }}", i * 2, i * 2 + 1)); }
        input.push_str("]}");
        assert!(classify_json(input.as_bytes()).contains("window_limit_exceeded"));
    }
}
#[cfg(not(test))]
#[cfg(not(test))]
