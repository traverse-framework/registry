...
mod tests {
    ...
    #[test]
    fn applies_scale_and_offset_with_clamping() {
        let result = calibrate(br#"{"evidence_id":"e1","raw_score_millis":500,"scale_millis":1200,"offset_millis":25}"#);
        assert!(result.contains("\"calibrated_score_millis\":625"));
        assert!(result.contains("\"result_class\":\"calibrated\""));

        let high = calibrate(br#"{"evidence_id":"e2","raw_score_millis":1000,"scale_millis":2000,"offset_millis":1000}"#);
        assert!(high.contains("\"calibrated_score_millis\":1000"));
        let low = calibrate(br#"{"evidence_id":"e3","raw_score_millis":0,"scale_millis":0,"offset_millis":-1000}"#);
        assert!(low.contains("\"calibrated_score_millis\":0"));
    }

    #[test]
    fn rejects_out_of_range_and_missing_fields() {
        for input in [
            br#"{"raw_score_millis":1,"scale_millis":1}"#.as_slice(),
            br#"{"evidence_id":"x","raw_score_millis":1001,"scale_millis":1}"#.as_slice(),
            br#"{"evidence_id":"x","raw_score_millis":1,"scale_millis":2001}"#.as_slice(),
            br#"{"evidence_id":"x","raw_score_millis":1,"scale_millis":1,"offset_millis":1001}"#.as_slice(),
        ] { assert!(calibrate(input).contains("invalid_request")); }
    }

    #[test]
    fn parses_integer_and_string_edges() {
        assert_eq!(int(br#"{"n":-7}"#, b"\"n\""), Some(-7));
        assert_eq!(int(br#"{"n":"x"}"#, b"\"n\""), None);
        assert_eq!(string(br#"{"id":"abc"}"#, b"\"id\""), b"abc");
        assert_eq!(string(br#"{"id":3}"#, b"\"id\""), b"");
    }
}
#[cfg(not(test))]
#[cfg(not(test))]
