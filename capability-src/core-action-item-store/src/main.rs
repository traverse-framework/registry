//! core.action-item-store — put/get/delete a durable action-item record via host state.
//!
//! Relative state key is fixed schema property `record`. Resource id lives inside the value
//! (`id`, `title`, `status`, `owner_id`). Calls traverse_host::{state_put,state_get,state_delete}.
...
#[cfg(test)]
unsafe fn state_get(ptr: i32, len: i32) -> i32 {
    // Host-unit tests write into STATE_OUT directly (same buffer do_get reads).
    // Parsing out_ptr as a raw host address from the envelope is not portable.
    let _ = (ptr, len);
...
unsafe fn do_put(input: &[u8], out: &mut [u8]) -> usize {
    let Some(record) = extract_object_at_depth(input, b"\"record\"", 1) else {
        return write_result(
            out,
            false,
            b"invalid_input",
            None,
            None,
            br#"["precondition failed: record object required for put"]"#,
        );
    };
    let id = extract_string_at_depth(record, b"\"id\"", 1);
    let title = extract_string_at_depth(record, b"\"title\"", 1);
    let status = extract_string_at_depth(record, b"\"status\"", 1);
    let owner_id = extract_string_at_depth(record, b"\"owner_id\"", 1);
    if id.is_empty() || title.is_empty() || status.is_empty() || owner_id.is_empty() {
        return write_result(
            out,
            false,
            b"invalid_input",
            None,
            None,
            br#"["precondition failed: record.id/title/status/owner_id required"]"#,
        );
    }
...
fn trim_c_str(buf: &[u8]) -> &[u8] {
    ...
    // Prefer JSON object end if present without relying only on NUL.
...
mod tests {
    ...
    fn happy_put() {
        let out = run(
            r#"{"action":"put","record":{"id":"ai-1","title":"Ship Wave 2","status":"open","owner_id":"user-ada"}}"#,
        );
        assert!(out.contains("\"reason_code\":\"ok\""), "{out}");
        assert!(out.contains("\"accepted\":true"));
        assert!(out.contains("\"record\":{"));
        assert!(out.contains("\"id\":\"ai-1\""));
    }

    #[test]
    fn happy_get_found() {
        set_get_response(
            r#"{"found":true,"value":{"id":"ai-1","title":"Ship Wave 2","status":"open","owner_id":"user-ada"}}"#,
        );
    ...
    fn rejects_put_missing_fields() {
        let out = run(r#"{"action":"put","record":{"id":"ai-1","title":"","status":"open","owner_id":"u"}}"#);
        assert!(out.contains("\"reason_code\":\"invalid_input\""), "{out}");
    }
    ...
    fn put_host_error() {
        ...
        let n = unsafe {
            evaluate(
                br#"{"action":"put","record":{"id":"ai-1","title":"T","status":"open","owner_id":"u"}}"#,
                &mut out,
            )
        };
    ...
    fn helpers_edge_cases() {
        ...
        assert!(extract_object_at_depth(br#"{"record":null}"#, b"\"record\"", 1).is_none());
        assert!(extract_object_at_depth(br#"{"record":"x"}"#, b"\"record\"", 1).is_none());
        assert_eq!(
            extract_object_at_depth(br#"{"record":{"id":"x"}}"#, b"\"record\"", 1),
            Some(br#"{"id":"x"}"#.as_slice())
        );
        assert_eq!(
            extract_object_at_depth(br#"{"record":{"note":"a\"b","id":"x"}}"#, b"\"record\"", 1),
            Some(br#"{"note":"a\"b","id":"x"}"#.as_slice())
        );
        ...
        assert_eq!(extract_object_at_depth(br#"{"record": {"id":"z"}}"#, b"\"record\"", 1), Some(br#"{"id":"z"}"#.as_slice()));
        assert_eq!(extract_object_at_depth(br#"{"record":{"a":{"b":1}}}"#, b"\"record\"", 1), Some(br#"{"a":{"b":1}}"#.as_slice()));
        // unterminated object
        assert!(extract_object_at_depth(br#"{"record":{"id":"x""#, b"\"record\"", 1).is_none());
    }

    #[test]
    fn get_response_exact_buffer_len() {
        // Cover STATE_OUT fill branch where response fills without trailing NUL pad path nuance
        set_get_response(r#"{"found":false}"#);
        let out = run(r#"{"action":"get"}"#);
