...
pub extern "C" fn _start() { unsafe {
    ...
    let len = assign(&INPUT[..total], &mut OUTPUT);
    let mut written = 0usize;
    let v = IoVec { buffer: OUTPUT.as_ptr(), length: len };
    let _ = fd_write(1, &v, 1, &mut written);
} }

fn assign(request: &[u8], out: &mut [u8]) -> usize {
    let subject_id = string_after(request, b"\"subject_id\"");
    let previous_revision = int_after(request, b"\"previous_revision\"").unwrap_or(0);
    ...
    if subject_id.is_empty() { return error_json(out, b"subject_id is required"); }
    if policy_version.is_empty() { return error_json(out, b"policy.version is required"); }

    let revision = previous_revision + 1;
    let mut at = 0usize;
    at = copy(out, at, b"{\"subject_id\":\"");
    at = copy_json(out, at, subject_id);
    at = copy(out, at, b"\",\"revision\":");
    at = write_i32(out, at, revision);
    at = copy(out, at, b",\"supersedes_revision\":");
    at = write_i32(out, at, previous_revision);
...
mod tests {
    ...
    fn exercises_runtime_fixture_1() {
        let input = include_bytes!("../tests/uc01-existing.json");
        let mut out = [0u8; 65_536];
        let length = assign(input, &mut out);
        assert!(length > 0);
        assert!(length <= out.len());
    }
    #[test]
    fn exercises_runtime_fixture_2() {
        let input = include_bytes!("../tests/uc02-first.json");
        let mut out = [0u8; 65_536];
        let length = assign(input, &mut out);
        assert!(length > 0);
        assert!(length <= out.len());
    ...
    fn remains_total_for_truncated_and_corrupted_real_requests() {
        for fixture in [include_bytes!("../tests/uc01-existing.json").as_slice(), include_bytes!("../tests/uc02-first.json").as_slice()] {
            for end in 0..=fixture.len() {
                let mut out = [0u8; 65_536];
                let length = assign(&fixture[..end], &mut out);
                assert!(length <= out.len());
            }
            for index in 0..core::cmp::min(fixture.len(), 512) {
                ...
                corrupted[index] = b'?';
                let mut out = [0u8; 65_536];
                let length = assign(&corrupted, &mut out);
                assert!(length <= out.len());
            }
        }
    }
