//! doc-approval.policy-store — put/get/delete a durable approval policy via host state.
//!
//! Relative state key is fixed schema property `policy`. Resource id lives inside the value
//! (`policy_id`, `thresholds`, `required_roles`). Calls traverse_host::{state_put,state_get,state_delete}.
...
unsafe fn do_put(input: &[u8], out: &mut [u8]) -> usize {
    let Some(policy) = extract_object_at_depth(input, b"\"policy\"", 1) else {
        return write_result(
            out,
            false,
            b"invalid_input",
            None,
            None,
            br#"["precondition failed: policy object required for put"]"#,
        );
    };
    let policy_id = extract_string_at_depth(policy, b"\"policy_id\"", 1);
    let thresholds = extract_object_at_depth(policy, b"\"thresholds\"", 1);
    let required_roles = extract_array_at_depth(policy, b"\"required_roles\"", 1);
    if policy_id.is_empty() || thresholds.is_none() || required_roles.is_none() {
        return write_result(
            out,
            false,
            b"invalid_input",
            None,
            None,
            br#"["precondition failed: policy.policy_id/thresholds/required_roles required"]"#,
        );
    }
...
mod tests {
    ...
    fn happy_put() {
        let out = run(
            r#"{"action":"put","policy":{"policy_id":"pol-1","thresholds":{"confidence_min":"0.8","amount_max":"10000"},"required_roles":["approver","auditor"]}}"#,
        );
        assert!(out.contains("\"reason_code\":\"ok\""), "{out}");
        assert!(out.contains("\"accepted\":true"));
        assert!(out.contains("\"policy\":{"));
        assert!(out.contains("\"policy_id\":\"pol-1\""));
    }

    #[test]
    fn happy_put_empty_required_roles() {
        let out = run(
            r#"{"action":"put","policy":{"policy_id":"pol-2","thresholds":{"confidence_min":"0.5"},"required_roles":[]}}"#,
        );
        assert!(out.contains("\"reason_code\":\"ok\""), "{out}");
    }
    ...
    fn happy_get_found() {
        set_get_response(
            r#"{"found":true,"value":{"policy_id":"pol-1","thresholds":{"confidence_min":"0.8"},"required_roles":["approver"]}}"#,
        );
    ...
    fn rejects_missing_action() {
        let out = run(r#"{"policy":{"policy_id":"pol-1"}}"#);
    ...
    fn rejects_put_missing_fields() {
        let out = run(
            r#"{"action":"put","policy":{"policy_id":"","thresholds":{"confidence_min":"0.8"},"required_roles":[]}}"#,
        );
        assert!(out.contains("\"reason_code\":\"invalid_input\""), "{out}");
    }

    #[test]
    fn rejects_put_missing_required_roles() {
        let out = run(
            r#"{"action":"put","policy":{"policy_id":"pol-1","thresholds":{"confidence_min":"0.8"}}}"#,
        );
        assert!(out.contains("\"reason_code\":\"invalid_input\""), "{out}");
    }
    ...
    fn put_host_error() {
        ...
        let n = unsafe {
            evaluate(
                br#"{"action":"put","policy":{"policy_id":"pol-1","thresholds":{"confidence_min":"0.8"},"required_roles":[]}}"#,
                &mut out,
            )
        };
    ...
    fn helpers_edge_cases() {
        ...
        assert!(extract_object_at_depth(br#"{"policy":null}"#, b"\"policy\"", 1).is_none());
        assert!(extract_object_at_depth(br#"{"policy":"x"}"#, b"\"policy\"", 1).is_none());
        assert_eq!(
            extract_object_at_depth(br#"{"policy":{"policy_id":"x"}}"#, b"\"policy\"", 1),
            Some(br#"{"policy_id":"x"}"#.as_slice())
        );
        assert_eq!(
            extract_object_at_depth(
                br#"{"policy":{"note":"a\"b","policy_id":"x"}}"#,
                b"\"policy\"",
                1
            ),
            Some(br#"{"note":"a\"b","policy_id":"x"}"#.as_slice())
        );
        assert_eq!(
            extract_object_at_depth(br#"{"policy": {"policy_id":"z"}}"#, b"\"policy\"", 1),
            Some(br#"{"policy_id":"z"}"#.as_slice())
        );
        assert_eq!(
            extract_object_at_depth(br#"{"policy":{"a":{"b":1}}}"#, b"\"policy\"", 1),
            Some(br#"{"a":{"b":1}}"#.as_slice())
        );
        assert!(extract_object_at_depth(br#"{"policy":{"policy_id":"x""#, b"\"policy\"", 1)
            .is_none());
        assert_eq!(
            extract_array_at_depth(br#"{"required_roles":["a","b"]}"#, b"\"required_roles\"", 1),
            Some(br#"["a","b"]"#.as_slice())
        );
        assert_eq!(
            extract_array_at_depth(br#"{"required_roles":[]}"#, b"\"required_roles\"", 1),
            Some(br#"[]"#.as_slice())
        );
        assert_eq!(
            extract_array_at_depth(br#"{"required_roles": ["a"]}"#, b"\"required_roles\"", 1),
            Some(br#"["a"]"#.as_slice())
        );
        assert_eq!(
            extract_array_at_depth(br#"{"required_roles":["a\"b","c"]}"#, b"\"required_roles\"", 1),
            Some(br#"["a\"b","c"]"#.as_slice())
        );
        assert!(extract_array_at_depth(br#"{"required_roles":"x"}"#, b"\"required_roles\"", 1).is_none());
        assert!(extract_array_at_depth(br#"{"required_roles":["a""#, b"\"required_roles\"", 1).is_none());
        ...
        let with_arr = br#"{"required_roles":["a","b"],"action":"get"}"#;
