//! doc-approval.packet-store — put/get/delete a durable approval packet via host state.
//!
//! Relative state key is fixed schema property `packet`. Resource id lives inside the value
//! (`packet_id`, `document_id`, `status`, `reviewer_ids`). Calls traverse_host::{state_put,state_get,state_delete}.
...
unsafe fn do_put(input: &[u8], out: &mut [u8]) -> usize {
    let Some(packet) = extract_object_at_depth(input, b"\"packet\"", 1) else {
        return write_result(
            out,
            false,
            b"invalid_input",
            None,
            None,
            br#"["precondition failed: packet object required for put"]"#,
        );
    };
    let packet_id = extract_string_at_depth(packet, b"\"packet_id\"", 1);
    let document_id = extract_string_at_depth(packet, b"\"document_id\"", 1);
    let status = extract_string_at_depth(packet, b"\"status\"", 1);
    let reviewer_ids = extract_array_at_depth(packet, b"\"reviewer_ids\"", 1);
    if packet_id.is_empty() || document_id.is_empty() || status.is_empty() || reviewer_ids.is_none() {
        return write_result(
            out,
            false,
            b"invalid_input",
            None,
            None,
            br#"["precondition failed: packet.packet_id/document_id/status/reviewer_ids required"]"#,
        );
    }
...
mod tests {
    ...
    fn happy_put() {
        let out = run(
            r#"{"action":"put","packet":{"packet_id":"pkt-1","status":"pending","document_id":"doc-9","reviewer_ids":["rev-a","rev-b"]}}"#,
        );
        assert!(out.contains("\"reason_code\":\"ok\""), "{out}");
        assert!(out.contains("\"accepted\":true"));
        assert!(out.contains("\"packet\":{"));
        assert!(out.contains("\"packet_id\":\"pkt-1\""));
    }

    #[test]
    fn happy_put_empty_reviewer_ids() {
        let out = run(
            r#"{"action":"put","packet":{"packet_id":"pkt-2","status":"pending","document_id":"doc-9","reviewer_ids":[]}}"#,
        );
        assert!(out.contains("\"reason_code\":\"ok\""), "{out}");
    }
    ...
    fn happy_get_found() {
        set_get_response(
            r#"{"found":true,"value":{"packet_id":"pkt-1","status":"pending","document_id":"doc-9","reviewer_ids":["rev-a"]}}"#,
        );
    ...
    fn rejects_missing_action() {
        let out = run(r#"{"packet":{"packet_id":"pkt-1"}}"#);
    ...
    fn rejects_put_missing_fields() {
        let out = run(
            r#"{"action":"put","packet":{"packet_id":"pkt-1","document_id":"","status":"pending","reviewer_ids":[]}}"#,
        );
        assert!(out.contains("\"reason_code\":\"invalid_input\""), "{out}");
    }

    #[test]
    fn rejects_put_missing_reviewer_ids() {
        let out = run(
            r#"{"action":"put","packet":{"packet_id":"pkt-1","status":"pending","document_id":"doc-9"}}"#,
        );
        assert!(out.contains("\"reason_code\":\"invalid_input\""), "{out}");
    }
    ...
    fn put_host_error() {
        ...
        let n = unsafe {
            evaluate(
                br#"{"action":"put","packet":{"packet_id":"pkt-1","status":"pending","document_id":"doc-9","reviewer_ids":[]}}"#,
                &mut out,
            )
        };
    ...
    fn helpers_edge_cases() {
        ...
        assert!(extract_object_at_depth(br#"{"packet":null}"#, b"\"packet\"", 1).is_none());
        assert!(extract_object_at_depth(br#"{"packet":"x"}"#, b"\"packet\"", 1).is_none());
        assert_eq!(
            extract_object_at_depth(br#"{"packet":{"packet_id":"x"}}"#, b"\"packet\"", 1),
            Some(br#"{"packet_id":"x"}"#.as_slice())
        );
        assert_eq!(
            extract_object_at_depth(
                br#"{"packet":{"note":"a\"b","packet_id":"x"}}"#,
                b"\"packet\"",
                1
            ),
            Some(br#"{"note":"a\"b","packet_id":"x"}"#.as_slice())
        );
        assert_eq!(
            extract_object_at_depth(br#"{"packet": {"packet_id":"z"}}"#, b"\"packet\"", 1),
            Some(br#"{"packet_id":"z"}"#.as_slice())
        );
        assert_eq!(
            extract_object_at_depth(br#"{"packet":{"a":{"b":1}}}"#, b"\"packet\"", 1),
            Some(br#"{"a":{"b":1}}"#.as_slice())
        );
        assert!(extract_object_at_depth(br#"{"packet":{"packet_id":"x""#, b"\"packet\"", 1)
            .is_none());
        assert_eq!(
            extract_array_at_depth(br#"{"reviewer_ids":["a","b"]}"#, b"\"reviewer_ids\"", 1),
            Some(br#"["a","b"]"#.as_slice())
        );
        assert_eq!(
            extract_array_at_depth(br#"{"reviewer_ids":[]}"#, b"\"reviewer_ids\"", 1),
            Some(br#"[]"#.as_slice())
        );
        assert_eq!(
            extract_array_at_depth(br#"{"reviewer_ids": ["a"]}"#, b"\"reviewer_ids\"", 1),
            Some(br#"["a"]"#.as_slice())
        );
        assert_eq!(
            extract_array_at_depth(br#"{"reviewer_ids":["a\"b","c"]}"#, b"\"reviewer_ids\"", 1),
            Some(br#"["a\"b","c"]"#.as_slice())
        );
        assert!(extract_array_at_depth(br#"{"reviewer_ids":"x"}"#, b"\"reviewer_ids\"", 1).is_none());
        assert!(extract_array_at_depth(br#"{"reviewer_ids":["a""#, b"\"reviewer_ids\"", 1).is_none());
        ...
        let with_arr = br#"{"reviewer_ids":["a","b"],"action":"get"}"#;
