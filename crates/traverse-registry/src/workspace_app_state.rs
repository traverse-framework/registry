...
mod tests {
    ...
    fn write_relative_workspace_artifacts(workspace_root: &Path) {
        ...
        fs::copy(
            repo.join(
                "crates/traverse-registry/tests/fixtures/component-artifact.txt",
            ),
    ...
    fn expedition_components_json(repo: &Path) -> Vec<Value> {
        ...
        .map(|(leaf, capability_id)| {
            serde_json::json!({
                ...
                "artifact_ref": repo.join("crates/traverse-registry/tests/fixtures/component-artifact.txt").display().to_string()
            })
        })
        .collect()
    }
