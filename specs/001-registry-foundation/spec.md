...
- **FR-011**: The capability record schema MAY include a `use_cases` field, an array of `{scenario, input_example, output_example, happy}` objects describing concrete scenarios the capability handles — a search-focused excerpt of a capability's behavior, distinct from and narrower than any fuller supplementary documentation (e.g. a `SPEC.md`) a publisher may keep alongside their source. Existing records without this field remain valid; adding it to a new version is an additive (minor) schema change. Each `scenario` MUST be phrased as a full user story — `"As a <persona>, I want to <action>, so that <benefit>."` — not a plain declarative sentence; the persona and benefit clauses are what make a scenario useful for discovery (matching it to a real workflow need, per decision-log entry 42), not just a restatement of the capability's own description.
