...
**Status**: Approved
**Version**: 1.4.0
**Input**: Brainstorm session establishing `traverse-framework/registry` as the public capability registry for Traverse, extracted from `traverse-framework/traverse`'s `crates/traverse-registry`. Full narrative record: `docs/decision-log.md`. Companion migration record: `traverse-framework/traverse` `specs/051-registry-extraction/spec.md`. FR-011 amended 2026-08-10 (Decision 55 / traverse Decision 58): `use_cases` MUST for newly ADDED or CHANGED contracts with full schema surface coverage. FR-016 added 2026-08-10 (Decision 56 / registry#168): first-class `events/` publish path and index `events[]` for ECCA event products. FR-017 added 2026-09-10 (Decision 104 / registry catalog agent-marker brainstorm): optional `ai` object on the capability schema; `ai.model_backed: true` is the definition of an "agent". FR-017 amended 2026-09-16 (Decision 124 / owner `/brainstorm` / registry#571): `ai.models` gains an object-shaped `ModelRef[]` alternative to the original `string[]`, carrying pinned HF/provenance attribution (id, SPDX expression, attribution flag, pinned huggingface_id+revision or source_url); required on newly ADDED `model_backed` contracts, legacy `string[]` grandfathered forever; the contract is the source of truth for model attribution, not the catalog.
...
- **FR-011**: The capability record schema MUST include a non-empty `use_cases` field on every newly ADDED or CHANGED `contract.json`, an array of `{scenario, input_example, output_example, happy, persona_ref}` objects describing concrete scenarios that cover the capability's declared schema surface (traverse Spec `102-contract-surface-coverage` / Decision 58) — a search-focused, executable excerpt of behavior, distinct from and narrower than any fuller supplementary documentation (e.g. a `SPEC.md`) a publisher may keep alongside their source. Already-published immutable versions that predate this requirement remain readable without rewrite; correcting them requires an honesty patch-bump (new version), not an in-place edit. Each `scenario` MUST be phrased as a full user story — `"As a <persona>, I want to <action>, so that <benefit>."` — not a plain declarative sentence; the persona and benefit clauses are what make a scenario useful for discovery (matching it to a real workflow need, per decision-log entry 42), not just a restatement of the capability's own description.
- **FR-012**: A capability's `namespace` MUST be either (a) the reference app it belongs to, per the app-scoped convention `specs/008-reference-capability-publication/spec.md` already establishes for reference-app capabilities, or (b) `core`, for any core-team-published capability with no natural reference app to scope to. `namespace` MUST NOT be used to encode topic/category — that is the `use_cases`/`summary`/`description` fields' role, not namespace's, per FR-008's original intent (identifying a publisher, reserved for eventual third-party use) rather than a second, overlapping classification axis.
- **FR-013**: This repo MUST support publishing workflows as first-class, versioned, immutable records at `workflows/<namespace>/<id>/<version>/workflow.json`, laid out and governed the same way capability records are (FR-001, FR-002). The public index MUST include a `workflows[]` array alongside `capabilities[]`, built and published the same way (FR-006). A workflow record's list of the capability references it composes IS the canonical statement of what capabilities/versions belong together for a given use case — a separate "content group" record type MUST NOT be created to duplicate that same membership information. A capability with no natural workflow (a single-capability entrypoint) MAY instead carry a boolean `entrypoint` marker directly on its own contract.
- **FR-014**: A capability version known to be a fixed-output fixture or placeholder stub MUST be deprecated (FR-009) in the same PR that publishes its real, input-dependent successor — never left coexisting, undeprecated, alongside a real successor. This formalizes, as an explicit requirement, a practice already followed for every capability published in this repo to date; it does not describe new resolution behavior — semver-range resolution already excludes deprecated versions (FR-009), so consistently applying this rule is what makes a `^1`-style range reliably resolve to real logic, not a fixture.
- **FR-015**: The capability record schema MAY include a `runtime_owned_output_fields` field, an array of output field names that are populated by the runtime executing the capability (e.g. a trace id, a timestamp) rather than determined by the capability's own logic — distinguishing these from fields a consumer (such as an LLM prompt-builder) might otherwise mistake for genuine capability output. Defaults to empty; adding it is an additive (minor) schema change.
- **FR-016**: This repo MUST support publishing ECCA event products as first-class, versioned, immutable records at `events/<namespace>/<id>/<version>/product.json`, laid out and governed the same way capability records are (FR-001, FR-002). Each `product.json` MUST be an `EventProductDescriptor` (contract plus ECCA-additive fields: support_route, exposure, field_classifications, CloudEvents/delivery mappings, retention_policy) validated by deterministic CI. The public index MUST include an `events[]` array alongside `capabilities[]` and `workflows[]`, built and published the same way (FR-006).
- **FR-017**: The capability record schema MAY include an `ai` object — `{ model_backed: boolean, models?: string[] | ModelRef[] }` — declaring whether the capability performs model inference at execution time. `model_backed: true` means the capability runs a forward pass or invokes a model as part of producing its output (an in-process neural runtime, or a host/network model call); static lookup tables, quantized weights, or embeddings compiled into the WASM artifact and used only for deterministic arithmetic do NOT qualify and MUST NOT set it. When `model_backed` is `true`, `models` MUST be a non-empty array, in one of two shapes:
  - **Legacy `string[]`** (model-identifier strings, e.g. `"minishlab/potion-base-32M"`) — grandfathered permanently on already-published immutable versions; never required to change.
  - **`ModelRef[]`** (added 2026-09-16, Decision 124 / registry#571 — object-shaped, pinned HF/provenance attribution): each entry is an object `{ id: string, spdx_expression: string, attribution_required: boolean, huggingface_id?: string, revision?: string, source_url?: string, copyright?: string }`, where `spdx_expression` is a syntactically valid SPDX license expression and each entry MUST supply either (`huggingface_id` AND `revision`) — a pinned revision, not a moving `main`/`latest` ref — or `source_url`.

  A newly ADDED contract with `ai.model_backed: true` MUST use the `ModelRef[]` shape; the legacy `string[]` shape is only valid on contracts that predate this amendment. This is a forward gate (same shape as FR-010-style activations elsewhere in this registry) — already-published versions are never retroactively judged. The capability contract's `ai.models` is the **source of truth** for model license/attribution; any catalog-level attribution file (e.g. `catalog/model-attribution.json`) is at most a fallback display aid for legacy string-shaped entries, never authoritative over the contract. CI validates `ModelRef[]` shape and SPDX syntax only — it does NOT live-fetch Hugging Face or `source_url` at validation time (pin, don't live-fetch); an advisory drift check against the live Hub is an optional future addition, not required for merge. The `ai` object defaults to absent; a contract with no `ai` object, or with `model_backed: false`, is a plain deterministic capability. A capability whose contract declares `ai.model_backed: true` is, by definition, an **agent** (the term reserved in Decision 41); this registry has no agents until such a contract is published. CI (FR-003) MUST reject a malformed `ai` object (missing or non-boolean `model_backed`; malformed `models` entries in either shape; `model_backed: true` with an absent or empty `models`). The public index (FR-006) MUST carry each capability's `ai` declaration alongside the other per-entry projection fields, absent when the contract omits it. Already-published immutable versions that predate this field, or that predate the `ModelRef[]` shape, remain valid unchanged. Adding this field was an additive (minor) schema change; adding the `ModelRef[]` shape is likewise additive (minor) — no existing contract's meaning changes.
...
- **Event Product Record**: a `product.json` file at `events/<namespace>/<id>/<version>/`, an `EventProductDescriptor` wrapping a governed event contract with ECCA-additive publish metadata (spec `016-ecca-event-product-adoption`).
