...
**Created**: 2026-08-05
**Status**: Draft
**Input**: Registry-side adoption of `traverse-framework/traverse`'s `Spec 534` ("ECCA Event Products") and `ADR-0028` ("ECCA Event-Product Standard"), both Accepted 2026-07-29, to unblock `traverse#896` ("Implement ECCA registry catalog and capability discovery"), a child of `traverse#894` and gated on `traverse#895` (the spec/ADR ticket, now Done). The most recent comment on `traverse#896` (from the repo owner) confirms the work belongs here: Traverse's own workspace has no registry crate. Unlike `011-capability-registry-adoption` and `013-inherited-registry-governance`, this spec was not decided via an interactive `/brainstorm` with the repo owner before drafting its functional requirements -- it was adopted directly from the ops request that triggered this work, grounded in the already-owner-approved `Spec 534`/`ADR-0028`. See the **Enforcement note** below for what that means for this spec's approval status.

**Enforcement note**: `crates/traverse-registry/` already exists and is governed by `011-capability-registry-adoption`, `012-event-registry-adoption`, and `013-inherited-registry-governance`. This spec is a new, dedicated spec for new behavior, per `013`'s own `FR-002` ("any new requirement or behavior change from this point forward MUST go through a new, dedicated spec in this repo... not an edit to this spec or the specs it inherits from"). Per the registry-ops operating model's guardrail ("never move a spec from Draft to Approved on your own judgment -- requires the repo owner's explicit, standalone sign-off"), this spec is registered in `specs/governance/approved-specs.json`'s `draft_specs[]` array, not `specs[]`, and remains **Draft** until the repo owner approves it directly. Implementation may proceed against a Draft spec (precedent: decision log entry 22, `007-artifact-hosting` implemented while Draft) -- registering ahead of full FR-satisfaction, and even ahead of approval, is normal spec-driven order here, not a claim that this spec is already binding.
